...
namespace gcm {
    ...
    void build_cfg_breadth(Node* stop, Node** ns, Node** late) {
        ...
        while((n = work.pop()) != nullptr) {
            ...
            if(node::cfg(n)) {
                late[n->uid] = node::is_block_head(n) ? n : n->ctrl();
            } else if(n->nt == NodeType::Phi) {
                late[n->uid] = ((NodePhi*)n)->region();
            } else if(n->nt == NodeType::Proj && n->ctrl()->cfg()) {
                late[n->uid] = n->ctrl();
            } else {
                // All uses done?
    ...
    void schedule_node_early(Node* n, BitSet& visit) { todo; // need to make every node have "input[0]" to be the ctrl input
        ...
        for(u32 i = 1; i < n->input.size; i++) {
            ...
            if(n->input[i]->nt != NodeType::Phi) // phi because of backedge garbage
    ...
    void schedule_node_late(Node* n, Node** ns, Node** late) {
        ...
        Node* early = n->ctrl(); // get this node's early schedule
